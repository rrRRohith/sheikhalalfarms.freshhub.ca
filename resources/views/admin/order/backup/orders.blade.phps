                                 ...
                                 <div class="table-list-responsive-md">
                                    <table class="table table-customer mt-0">
                                       <thead class="table-list-header-options">
                                          <tr>
                                             <td colspan="8">
                                                  <form action="" method="get" class="order_search">
                                                     @csrf
                                                        <div class="row">
                                                           <!--<div class="col-sm-3">-->
                                             ...
                                             <td class="text-right">
                                                 ...
                                                 <select class="action" data-id="{{$order->id}}">
                                                        ...
                                                        @foreach($status as $s)
                                                         ...
                                                         @endforeach
                                                         <!--@foreach($status as $s)-->
                                                         <!--    @if($order->status < $s->id && $order->status+2 > $s->id && $s->id !=6 && $s->id !=4)-->
                                                         <!--    <option value="{{$s->id}}">{{$s->action}}</option>-->
                                                         <!--    @endif-->
                                                         <!--    @if($order->status <=1 && $s->id ==6)-->
                                                         <!--    @can('Order Delete')-->
			...
			<table>
				...
				<tbody id="order-items">
					<tr id="r-0">
						...
						<td>
							<input type="number" name="quantity[]" id="quantity-0" onkeyup="getTotal(0);" required/>
						</td>
						<td>
							<input type="text" name="rate[]" id="rate-0" onkeyup="getTotal(0);"/>
						</td>
						<td>
							<span id="amount-0"></span>
						</td>
						<td>
							<a href=""><button class="delete" id="delete-0" onclick="deteteRow(0);"><i class="fa fa-trash" aria-hidden="true"></i>
</button></a>
						</td>
					</tr>
			...
			<table>
				...
				<tbody id="order-items">
					<tr id="acceptr-0">
						...
						<td>
							<input type="text" name="rate1[]" id="rate1-0" onkeyup="getTotal1(1);" placeholder="Rate" required/>
						</td>
						<td>
...
function addBlankRow(i)
{
    ...
    if(id < Number(i)+2)
    {
        $('#id').val(id);
        var i=Number(id)+1;
        var data ='<tr id="r-'+id+'"><td><button class="tickmark"></button></td><td><span class="order-item">'+i+'</span></td><td><input type="text" name="product_name[]" id="product_name-'+id+'" onkeyup="newProduct('+id+');"><input type="hidden" name="product_id[]" id="product_id-'+id+'"><div id="result-div-'+id+'" style="display:none;background: darkturquoise;"></div></td><td><input type="text" name="description[]" id="description-'+id+'"/></td><td><input type="number" name="quantity[]" id="quantity-'+id+'" onkeyup="getTotal('+id+');"/></td><td><span id="creditrate-'+id+'"></span><input type="hidden" name="rate[]" id="rate-'+id+'" onkeyup="getTotal(1);"/></td><td><span id="amount-'+id+'"></span></td><td><button class="delete" id="delete-'+id+'" onclick="deteteRow('+id+');"></button></td></tr>';
...
function addBlankRowAccept(i)
{
    ...
    if(id < Number(i)+2)
    {
        $('#acceptid').val(id);
        var i=Number(id)+1;
        var data ='<tr id="acceptr-'+id+'"><td><button class="tickmark"></button></td><td><span class="order-item">'+i+'</span></td><td><span id="product_name1-'+id+'"></span><input type="hidden" name="product_id1[]" id="product_id1-'+id+'"><div id="result-div-'+id+'" style="display:none;background: darkturquoise;"></div></td><td><span id="reqqty1-'+id+'"></span></td><td><span id="stockqty1-'+id+'"></span><input type="hidden" name="reqty" id="reqty-'+id+'"></td><td><input type="number" name="quantity1[]" id="quantity1-'+id+'" placeholder="Quantity" required/></td><td><input type="text" name="rate1[]" id="rate1-'+id+'" onkeyup="getTotal1('+id+');" placeholder="Rate"/></td><td><span id="amount1-'+id+'"></span></td></tr>';
...
function addStockRow()
{
    var id=parseInt($('#stockid').val())+1;
        ...
        $('#stockid').val(id);
        var i=Number(id)+1;
        var data ='<tr><td>'+i+'</td><td><span id="name-'+id+'"></span><input type="hidden" name="stockprod[]" id="stockprod-'+id+'"></td><td><input type="number" id="backqty-'+id+'" name="backqty[]" class="form-control"></td><td><input type="number" id="backqtyamount-'+id+'" name="backqtyamount[]" class="form-control" placeholder="Please enter amount"></td></tr>';
