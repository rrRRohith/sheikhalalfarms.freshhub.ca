...
<ul>
    ...
    @can('Purchase Order View')<li class="active"><a  href="{{admin_url('purchaseorders')}}">Purchase Orders</a>@endcan
      @can('Invoices View')<li><a  href="{{admin_url('invoices')}}">Invoices</a>@endcan
...
let deferUrl = `{{ admin_url('purchaseorders/defer')}}`;
...
//   });
  $('a.sort').on('click',async function(e){
    ...
    loadingRow();
    await $.getJSON(`${deferUrl}?key=${searchKey.val()}&byday=${searchByday.val()}&sort=${sort}&direction=${direction}`, function(response) {
      renderTable(response);
...
{
    $.getJSON(`${deferUrl}?key=${searchKey.val()}&byday=${searchByday.val()}`, function(response) {
       renderTable(response);
     });
}
async function renderTable(response){
  ...
  await response.data.forEach(function(row, index){
    ...
    {
        text='<a href="order/printorder/'+row.id+'" target="_blank"><li>Print Order</li></a><a href="#" class="process-order" data-id="'+row.id+'"><li>Process Order</li></a>@can("Order Edit")<a href="#" class="edit-order" data-id="'+row.id+'"><li>Edit Order</li></a>@endcan';
    }
    ...
    table+=`<tr><td>
            ...
            <td class="text-lg-center  text-md-center" data-label="Organization" style="font-size:16px;">
              ${row.status==1 ? '<span class="badge bg-warning">New Order</span>' : (row.status==2 ? '<span class="badge bg-success"> Accepted</span>':(row.status==3 ? '<span class="badge bg-info"> Processing</span>':(row.status==4 ? '<span class="badge bg-warning"> Ready</span>':(row.status==5 ? '<span class="badge bg-success"> Dispatching</span>':(row.status==6 ? '<span class="badge bg-danger"> Delivered</span>':(row.status==0 ? '<span class="badge bg-danger"> Backorder</span>':'<span class="badge bg-black"> Cancelled </span>')))))) } 
            </td>
